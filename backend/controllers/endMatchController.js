...
const endMatch = async (req, res) => {
  try {
    ...
    if (!match.winner) {
      // FIXED: Handle case where innings2 might not exist (match ended during innings 1)
      const innings1Runs = match.innings1?.runs || 0;
      const innings2Runs = match.innings2?.runs || 0;

      // Only calculate winner if innings2 exists and has runs
      if (match.innings2 && innings2Runs > innings1Runs) {
      ...
      } else if (match.innings2 && innings1Runs > innings2Runs) {
